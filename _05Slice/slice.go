...
func main() {
	/*切片（Slice）是一个拥有相同类型元素的可变长度的序列。它是基于数组类型做的一层封装。它非常灵活，支持自动扩容。
	  注意：切片是一种变长数组，切片是一个引用类型，进行函数参数传递时会影响切片数据，而数组进行函数参数传递是值类型，不会影响数组数据
	  它的内部结构包含地址、长度和容量，可以将切片理解为在底层的数组上进行操作，容量为给该切片在底层数组上分配了多少个可使用的存储空间，长度为实际已使用的存储空间。
	  切片的拷贝与从同一个数组上生成的两个切片，其底层指向同一个数组，因此修改具有统一性。
	*/
	//声明一个整型切片，但需要注意的是：直接声明切片类型的变量是没有意义的
	var a []int
	//声明一个字符串切片并初始化
	var b = []string{}
	...
	/*切片的创建一般有两种方式：
	  1、通过数组创建
	  2、通过make函数创建：各元素被默认初始化为切片元素类型的零值*/
